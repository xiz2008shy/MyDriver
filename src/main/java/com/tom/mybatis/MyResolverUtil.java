...
public class MyResolverUtil<T> {
     ...
     *
     * @param test
     *          an instance of {@link MyResolverUtil.Test} that will be used to filter classes
     * @param packageName
    ...
    public MyResolverUtil<T> find(MyResolverUtil.Test test, String packageName) {
        log.info("MyResolverUtil.find packageName="  + packageName);
        String path = getPackagePath(packageName);
        log.info("MyResolverUtil.find path="  + path);
        try {
            List<String> children = VFS.getInstance().list(path);
            log.info("MyResolverUtil.find children.size="  + children.size());
    ...
    protected void addIfMatching(MyResolverUtil.Test test, String fqn) {
        try {
            ...
            log.info("Checking to see if class " + externalName + " matches criteria [" + test + "]");
            ...
            log.error("Could not examine class '" + fqn + "'" + " due to a " + t.getClass().getName() + " with message: "
                    + t.getMessage());
        }
    }
}
