...
![20240423](http://8.142.121.115:8080/crm_pack/v20240423.png)
...
### 20240501
界面上这段时间没有太大的变化，补充了更多需要的配置项，这两天开始着手具体同步同能的实现
1.MD5的部分摘要算法准备中
2.mybatis-mapper自动归还连接池处理完成
3.本地文件缺失或远端记录缺失的检查完成
4.界面上oss相关配置，和启用同步相关开关添加完成
![20240501-1](http://8.142.121.115:8080/crm_pack/v20240501-1.png)
![20240501-2](http://8.142.121.115:8080/crm_pack/v20240501-2.png)

### 20240503
本地和远端的差异操作主体框架完成
然后意识到还有一部分本地操作导致的差异需要先本地做一次比对，正好和之前构想的本地数据记录关联起来。

这件事其实是说，假如本地文件执行了删除之类的操作，处理的时候光是比对远端和本地文件，就只会发现本地文件不存在，需要去同步远端文件。
这个问题解决有几种方式，一种就是在执行删除动作时，同时删除远端数据，但删除文件这个动作其实是操作系统的能力，要完全处理掉还需要监听到操作系统的删除事
件，并且需要这样的监听持续进行，软件不能关，这点显然违背了用户意愿。第二种方式就只是在软件中提供一个删除操作，执行的时候会主动清理远端记录，
但这样的方式不能覆盖掉全部场景，就会出现用户发现删除的文件重新恢复的情况。所以我设想的方式是添加一个本地记录，本地记录对本地文件扫描时产生的持久化文件，记录了
目录下文件的部分元数据信息，在每次扫描时现价本地文件同本地记录进行比对，归纳出本地执行的操作，再对本地执行的文件操作进行远端同步，就可以比较好的解决掉这个问题。
类似的问题像是文件重命名/移动，目录重命名/移动也可以在通过上述方式判断后，进优化操作，而不需要每次传输文件